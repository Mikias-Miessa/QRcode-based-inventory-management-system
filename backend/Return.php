<?php
include 'conn.php';
if(isset($_POST['qrCodeMessage'])){

    $qrCodeMessage= $_POST['qrCodeMessage'];

    $sql="SELECT * FROM products WHERE SKU='$qrCodeMessage'";
    $result=mysqli_query($con,$sql);
    // $data = mysqli_fetch_array($result, MYSQLI_NUM);
    //  if(!empty($data)) {
        $response=array();
    while($row=mysqli_fetch_assoc($result)){
        $response=$row;
    }
    echo json_encode($response);
    }
    // else{
    //     // $response=array();
    //     $response = array('Product_Name' => "Not registered", 'Product_Type' => "Not registered", 'Shelf_Number' => "Not Registered");
    //    echo json_encode($response);
    // }

   
// }
else{
    $response['status']=200;
    $response['message']="Data not found";
}

if(isset($_POST['sku'])&&isset($_POST['productquantity'])){
    $productquantity=$_POST['productquantity'];
    $sku=$_POST['sku'];
    $Stockindate=$_POST['Stockindate'];
    $returnedby=$_POST['returnedby'];
    $RETURNEDBY=strtoupper($returnedby);


    $sql="UPDATE products SET Quantity= Quantity+$productquantity WHERE SKU='$sku' ";
    $result=mysqli_query($con,$sql);

    $sql="INSERT INTO stockin_date(Stockin_Date,SKU,Quantity)VALUES('$Stockindate','$sku','$productquantity')";
    $res=mysqli_query($con,$sql);
    
    // $sql1="SELECT * FROM issued_products WHERE SKU='$sku' AND Issued_To='$returnedby'";
    // $res1=mysqli_query($con,$sql1);
    // while ($row = mysqli_fetch_assoc($res1)) {
    //     $product_quantity = $row['Quantity'];
    // }
    // if($productquantity = $product_quantity){
    // $query="DELETE FROM issued_products WHERE Issued_To='$returnedby' AND SKU='$sku' ";
    // $ress=mysqli_query($con,$query);
    // }
    // else{
    //     $query="UPDATE issued_products SET Quantity= Quantity-$productquantity WHERE SKU ='$sku' "; 
    //     $res=mysqli_query($con,$query);
    // }
    $sql="DELETE FROM issued_products WHERE SKU='$sku' AND Issued_To='$RETURNEDBY' AND Quantity=$productquantity";
    $res=mysqli_query($con,$sql);

    $sql="UPDATE issued_products set Quantity=Quantity-$productquantity WHERE SKU='$sku' AND Issued_To='$RETURNEDBY'";
    $result=mysqli_query($con,$sql);
}
    
    

?>
